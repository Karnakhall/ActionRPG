...
void AItem::BeginPlay()
{
	...
	//zamiast dw�ch powy�szych linijek makro, poni�ej zosta�a stworzona jedna linijka makro kt�a zawiera dwie powy�sze instrukcje w sobie
...
{
	/* Poni�szy kod sprwdza czy actor overlappuje z naszym przedmiotem i wy�wietlamy informacj� na ekranie
	...
	*/
	...
	//Sprawdzamy czy bohater overlappuje z naszym przedmiotem, jesli tak, mo�emy podnie�� przedmiot
...
void AItem::Tick(float DeltaTime)
{
	...
	if (ItemState == EItemState::EIS_Hovering)	//Je�li nasz przedmiot jest w stanie Hovering, to unosimy go i opuszczamy
	{
		//Hovering item
	...
	AddActorWorldOffset(FVector(MovementRate * DeltaTime, 0.f, 0.f));			// Przesuwamy "aktora" o 50 w osi X, bez wzgl�du na to ile FPS ma dany komputer
	...
	*/
	...
	//float DeltaZ = Amplitude * FMath::Sin(RunningTime * TimeConstant);		// period = 2*pi/K  funkcja powoduj�ca unoszenie si� i opadanie danego obiektu
	...
	// FRotator AvgRotator = Avg<FRotator>(GetActorRotation(), FRotator::ZeroRotator);  W tym przypadku nasza funkcja nie dzia�a poniewa� w odniesieniu nie ma wbudowanego dzielenia, wi�c nie mo�emy go wykorzystac w tym przypadku
	
	/*Informacje wy�wietlacj�ce w output log oraz na ekranie
